...
abstract class ApiObject implements IApiObject
{
    ...
    public function __construct(array $data, LeagueAPI $api = null)
    {
        ...
        $iterableProp = $selfRef->hasProperty('_iterable')
            ...
            : false;
        $linkableProps = $selfRef->hasProperty('staticData')
            ? self::getLinkablePropertyData($selfRef->getDocComment(), $selfRef)
        ...
        foreach ($data as $property => $value) {
            try {
                ...
                if ($api) {
                    foreach ($linkableProps as $linkableProp) {
                        ...
                        if ($linkableProp['parameter'] == $property && $api->getSetting(LeagueAPI::SET_STATICDATA_LINKING, false)) {
                            ...
                            $this->staticData[$linkableProp['parameter']] = $linkingFunctionRef->invokeArgs($api, $params);
                        }
                    }
     ...
     * @return bool|array
     */
    public static function getLinkablePropertyData(string $phpDocComment, $selfref)
    {
        preg_match_all('/@linkable\s(?<function>[\w]+)(?:\(\$(?<parameter>[\w]+)+?\))?/', $phpDocComment, $matches);
        ...
        if (@$matches['function'] && @$matches['parameter']) {
            ...
            if (is_array($matches['function'])) {
                $new_matches = [];

                for ($i = 0; $i < (count($matches['function'])); $i++) {
                    $new_matches[] = ['function' => $matches['function'][$i], 'parameter' => $matches['parameter'][$i]];
                }
